...
const MOCK_SEATS = {
  S: Array.from({ length: 14 }, (_, i) => ({
    seatNumber: i + 1,
    isOccupied: [1, 3, 4, 6, 7, 8, 10, 11, 14].includes(i + 1),
  })),
  A: Array.from({ length: 14 }, (_, i) => ({
    seatNumber: i + 1,
    isOccupied: [1, 2, 4, 5, 6, 8, 9, 10, 11, 12].includes(i + 1),
  })),
  B: Array.from({ length: 14 }, (_, i) => ({
    seatNumber: i + 1,
    isOccupied: [2, 3, 5, 6, 7, 9, 11, 12, 13].includes(i + 1),
  })),
  C: Array.from({ length: 14 }, (_, i) => ({
    seatNumber: i + 1,
    isOccupied: [1, 4, 5, 6, 8, 10, 11, 13, 14].includes(i + 1),
  })),
};
...
export default function HomePage() {
  ...
  const mySeat = user?.seat
    ...
    : null;
  const hasUnreadMessages = Array.isArray(messages)
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          >
            {mySeat ? (
              <>
                {`현재 이용 중인 좌석은 `}
                <strong style={{ color: ZQ.text2 }}>
                  {mySeat.zone}-{mySeat.seatNumber}번
                </strong>
                {`입니다.`}
              </>
            ) : (
