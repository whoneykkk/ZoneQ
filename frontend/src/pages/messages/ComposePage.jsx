...
export default function ComposePage() {
  ...
  const [selSeat, setSelSeat] = useState(
    initialZone && initialSeatNumber ? { zone: initialZone, seatNumber: initialSeatNumber, seatId: null } : null
  )
  ...
  useEffect(() => {
    if (seatData && selSeat && !selSeat.seatId) {
      const found = seatData[selSeat.zone]?.find((s) => s.seatNumber === selSeat.seatNumber)
      if (found) setSelSeat((prev) => prev ? { ...prev, seatId: found.id } : null)
    }
  }, [seatData])
    ...
    <div style={{ background: ZQ.bg, minHeight: '100vh' }}>
      <BackHeader
        ...
        rightEl={
          ...
          >{mutation.isPending ? '전송 중...' : '보내기'}</button>
        }
      />
