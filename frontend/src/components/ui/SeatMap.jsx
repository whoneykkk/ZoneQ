      ...
      >
        <span
          style={{
            fontSize: 11,
            fontWeight: 800,
            color: selected ? "#fff" : OCC_TEXT[zone],
            fontFamily: "'NanumSquare_ac', sans-serif",
          }}
        >
          {seat.seatNumber}
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      >
        {seats.map((s) => {
          const isMine =
            mySeat?.zone === zone && mySeat?.seatNumber === s.seatNumber;
          const selected =
            selSeat?.zone === zone && selSeat?.seatNumber === s.seatNumber;
          const handleClick =
            mode === "checkin"
              ? !s.isOccupied
                ? () => onSeatClick?.(zone, s.seatNumber, s.id)
                : undefined
              : s.isOccupied && !isMine
                ? () => onSeatClick?.(zone, s.seatNumber, s.id)
                : undefined;
